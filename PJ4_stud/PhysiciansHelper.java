...
public class PhysiciansHelper
{
	...
	public void readFile(Scanner data) {
		while(data.hasNextLine()) {
			...
			String symptomsPortion = line[1].toLowerCase();
			String[] symptoms = symptomsPortion.trim().split("\\s*,\\s*");
	...
	public void processSymptoms()
	{
		...
		if(!illnessToFrequency.isEmpty()) {
			System.out.println("Possible illnesses that match symptoms given include: \n");
		}
		for(int i=1; i<maxCounter+1; i++) {
			System.out.println("==> Diseases match " + i + "symptoms");
